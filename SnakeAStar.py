...
#A* is not working, fixed PriorityQueue unorderable objects error
...
def runGame():
    ...
    while True:
        ...
        #Generate list of directions for AI agent to trace the shortest path   
        directions = list()
        lastNode = path[0]
        path = path[1:]
        for node in path:
        ...
        #Display grid, AI agent, goal position
        ...
        #Move AI agent based on directions
        for direction in directions:
            ...
            #Update grid, AI agent, goal position
            DISPLAYSURF.fill(BGCOLOR)
            colorPath(nodeMap)
            drawGrid()
            drawGoal(goalNode)
